...
void heapify(keyType list[], int n, int i){
  ...
  if(l<n && compareName(list[l].name , list[largest].name) > 0 )
    largest = l;
  if (r<n && compareName(list[r].name , list[largest].name) > 0 )
    largest = r;
...
void selectionSort(keyType list[], int n){
  int i, j, min_index;
  for(i=0; i<n; i++){
    min_index = i;
    
    for(j=i+1; j<n; j++)
      if(compareName(list[j].name , list[min_index].name) < 0)
	min_index = j;
      
    swap(list[min_index].name, list[i].name);
    
  }
}

void quickSort(keyType list[], int left, int right){
  int  i, j;
  char pivot[MAX];
  if(left<right){
    i=left; j=right+1;
    ...
    do{
      do i++;
      while(compareName(pivot, list[i].name) > 0);
      do j++;
      while(compareName(pivot, list[j].name) < 0);
      swap(list[i].name, list[j].name);
    }while(i<j);
    swap(list[left].name, list[j].name);
