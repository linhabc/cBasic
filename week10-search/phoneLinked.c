...
//searching
phone_t searchMoveToFront(char model[], node_addr *root);
int main(){
  int a,n;
  phone_t* phone;
...
phone_t searchMoveToFront(char model[], node_addr *root){
  ...
  node_addr* p=root;
  phone_t tmp;
  for(p; p!=NULL && strcmp(p->ph.model,model)!=0; p=p->next)
    count++;
  if(count>0){
    tmp = p->ph;
    deleteAtPos(count);
...
/*
phone_t transPose(char model[], node_addr *root){
  int count=0;
  phone_t tmp;
  node_addr* p=root,prev;
  for(p; p!=NULL && strcmp(p->ph.model,model)!=0; p=p->next){
    count++;
    prev=p;
  }
